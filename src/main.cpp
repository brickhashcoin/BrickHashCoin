...
#if defined(NDEBUG)
# error "Brickhashcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x88692d525847f19b974f3646637db11fe0e22984e9c773ef371a34d650166438");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Brickhashcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Brickhashcoin Signed Message:\n";
...
{
    // Brickhashcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 500 * COIN;
            ...
            if(nHeight == 1)
            {
            nSubsidy = 92222222 * COIN;
            }

    // Subsidy is cut in half every 830000 blocks
    nSubsidy >>= (nHeight / 830000);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Brickhashcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Brickhashcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1471785907, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "The Times 21Aug2016 Chancellor on brink of second bailout for banks";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 500 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("049542e5a65e792af8c605177fd149be1790c9a7add95607dcf02f1a77094b074adb32ab0516cdb7e8f8790187a6c64ac31c279b5cd6c05b8ce5b5c1bfae8d1037") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1471785907;
        ...
        assert(block.hashMerkleRoot == uint256("0x6dd00a4fc0ea13e4904bd73813dca5e4524628d5b36e64f2ea2fe42e5738cfb6"));
...
unsigned char pchMessageStart[4] = { 0x0c, 0xfc, 0xd3, 0x9e }; // Brickhashcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BrickhashcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BrickhashcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BrickhashcoinMiner(CWallet *pwallet)
{
    printf("BrickhashcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("brickhashcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BrickhashcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BrickhashcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BrickhashcoinMiner, pwallet));
}
